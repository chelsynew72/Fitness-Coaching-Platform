    ...
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="mt-3 text-5xl font-black uppercase italic tracking-tighter mb-4">
            Find Your Coach
          </h1>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="h-72 bg-zinc-950 border border-white/5 rounded-xl shimmer" />
            ))}
          </div>
        ) : filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((coach, i) => {
              const isSubscribed = myCoachId === coach.userId?._id || myCoachId === coach.coachId;
              const isSubscribing = subscribing === (coach.userId?._id || coach._id);
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex gap-4 mb-4 py-3 border-t border-white/5">
                      <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                        ...
                        {coach.clients?.length || 0} clients
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                        <Dumbbell className="h-3.5 w-3.5" />
                        {coach.experience || 1}+ years
                      </div>
                    </div>
                    ...
                    {isSubscribed ? (
                      ...
                      <button
                        onClick={() => handleSubscribe(coach.userId?._id || coach._id)}
